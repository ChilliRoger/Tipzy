    ...
    <div className="mx-auto w-full container-max px-4 sm:px-6 lg:px-8 py-12">
      <section className="grid md:grid-cols-2 gap-8 items-center">
        <div>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <Link href="#" className="px-5 py-3 rounded-md font-medium border border-[--color-primary] text-[--color-primary] hover:bg-[--color-secondary]">
              Download App
            </Link>
          </div>
        </div>
        <div className="card p-6">
          ...
          <p className="mt-3 text-sm text-[--color-text]/70">Demo preview area</p>
        </div>
      </section>
      ...
      <section className="mt-16 grid md:grid-cols-2 gap-6">
        <div className="card p-6">
          ...
          <p className="mt-2 text-[--color-text]/80">
      ...
      <section className="mt-16 grid md:grid-cols-3 gap-6">
        {["Restaurant Owner", "Salon Barber", "Parking Customer"].map((role) => (
          <div key={role} className="card p-6">
            <p className="text-[--color-text]/80">
              “Tipzy made tipping seamless for our patrons and boosted staff morale.”
            </p>
            <p className="mt-3 text-sm text-[--color-text]/60">— {role}</p>
          </div>
        ))}
      </section>
    </div>
